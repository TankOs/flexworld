...
class ExampleWorldGate : public flex::lua::WorldGate {
	public:
		...
		uint32_t create_entity( const flex::FlexID& cls_id, const EntityPosition& position, const std::string& planet_id );
		uint32_t create_entity( const flex::FlexID& cls_id, uint32_t parent_id, const std::string& hook_id );
		uint32_t create_entity( const flex::FlexID& cls_id, uint32_t container_id );
