...
std::string SHA1::MakeHexDigest( const std::string& msg ) {
	SHA1 sha;
	sha.Input( msg.c_str(), static_cast<unsigned int>( msg.size() ) );
...
std::string SHA1::BuildSum( const std::string& filename ) {
	...
	// Read file and append bytes to SHA1 object.
	in.read( &ch, 1 );
	while( !in.eof() ) {
		sha1.Input( ch );
		in.read( &ch, 1 );
